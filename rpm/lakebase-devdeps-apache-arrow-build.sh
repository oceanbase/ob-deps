...
PROJECT_NAME=${2:-"lakebase-devdeps-apache-arrow"}
VERSION=${3:-"20.0.0"}
RELEASE=${4:-"1"}
...
if [[ "${ID}"x == "alinux"x ]]; then
    ...
    if [[ $VERSION == "20.0.0" ]]; then
        ...
        yum install -y devdeps-grpc-1.50.0
    ...
    if [[ $VERSION == "20.0.0" ]]; then
        ...
        yum install -y devdeps-protobuf-3.19.5
        yum install -y devdeps-grpc-1.50.0
        dep_pkgs=(obdevtools-gcc9-9.3.0-72024081318.el obdevtools-llvm-17.0.6-72025060300.el)
    fi
...
if [[ $VERSION == "20.0.0" ]]; then
    ...
    export GRPC_DEPS_DIR=/usr/local/oceanbase/deps/devel
    mv /usr/local/oceanbase/deps/devel/lib/grpc/* /usr/local/oceanbase/deps/devel/lib/
    mv /usr/local/oceanbase/deps/devel/lib64/grpc/* /usr/local/oceanbase/deps/devel/lib64/
else

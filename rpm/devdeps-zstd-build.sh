...
# check source code
if [[ -z `find $ROOT_DIR -maxdepth 1 -regex ".*/zstd-$VERSION.*[tar|gz|bz2|xz|zip]$"` ]]; then
    echo "Download ${PROJECT_NAME} source code"
    wget --no-check-certificate https://github.com/facebook/zstd/archive/refs/tags/v${VERSION}.tar.gz -O $ROOT_DIR/zstd-$VERSION.tar.gz
fi

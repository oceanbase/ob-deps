...
%define _compiler-rt_src compiler-rt-%{version}.src
...
cd $source_dir
rm -rf %{_llvm_src} %{_lld_src} %{_clang_src} %{_compiler-rt_src}
tar xf %{_llvm_src}.tar.xz
tar xf %{_lld_src}.tar.xz
tar xf %{_clang_src}.tar.xz
tar xf %{_compiler-rt_src}.tar.xz
...
mv -f %{_compiler-rt_src} $source_dir/llvm_src_dir/compiler-rt
...
cmake ../llvm -DCMAKE_EXE_LINKER_FLAGS="-static-libgcc -static-libstdc++" -DCMAKE_INSTALL_PREFIX=%{_relwithdebinfo_dir} -DLLVM_TARGETS_TO_BUILD=$arch -DLLVM_ENABLE_RTTI=ON -DLLVM_ENABLE_EH=ON -DCMAKE_BUILD_TYPE=RelWithDebInfo -DLLVM_ENABLE_PROJECTS='clang;compiler-rt' -G 'Unix Makefiles';
...
cmake ../llvm -DCMAKE_EXE_LINKER_FLAGS="-static-libgcc -static-libstdc++" -DCMAKE_INSTALL_PREFIX=%{_rel_dir} -DLLVM_TARGETS_TO_BUILD=$arch -DLLVM_ENABLE_RTTI=ON -DLLVM_ENABLE_EH=ON -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS='clang;compiler-rt' -G 'Unix Makefiles';
make -j${CPU_CORES};
make install;

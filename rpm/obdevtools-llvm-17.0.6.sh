...
# check source code
if [[ -z `find $ROOT_DIR -maxdepth 1 -regex ".*/clang+llvm-$VERSION.*[tar|gz|bz2|xz|zip]$"` ]]; then
    echo "Download ${PROJECT_NAME} source code"
    wget --no-check-certificate https://github.com/llvm/llvm-project/releases/download/llvmorg-17.0.6/clang+llvm-17.0.6-arm64-apple-darwin22.0.tar.xz -O $ROOT_DIR/clang+llvm-$VERSION.tar.xz
fi
...
cd $TMP_DIR
tar -xf $ROOT_DIR/clang+llvm-$VERSION.tar.xz
cd clang+llvm-17.0.6-arm64-apple-darwin22.0

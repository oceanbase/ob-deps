...
GCC_LINK_DIR_FLAGS="-B${GCC_LIB_DIR} -L${GCC_LIB_DIR} -L/usr/lib64"
CPP_INCLUDES="-isystem /usr/include/c++/${GCC_VER} -isystem /usr/include/c++/${GCC_VER}/${ARCH_TRIPLET} -isystem /usr/include"
export LIBRARY_PATH="${GCC_LIB_DIR}:/usr/lib64${LIBRARY_PATH:+:${LIBRARY_PATH}}"
export CFLAGS="-fPIC -s -mcmodel=large -D_GNU_SOURCE -fstack-protector-strong $DISABLE_ATOMIC -gdwarf-4 -flto=thin --gcc-toolchain=/usr -fuse-ld=lld ${GCC_LINK_DIR_FLAGS} -isystem /usr/include"
export CXXFLAGS="-std=c++17 -fPIC -s -mcmodel=large -D_GNU_SOURCE -D_GLIBCXX_USE_CXX11_ABI=0 -fstack-protector-strong $DISABLE_ATOMIC -gdwarf-4 -flto=thin --gcc-toolchain=/usr -fuse-ld=lld ${GCC_LINK_DIR_FLAGS} ${CPP_INCLUDES}"
export LDFLAGS="-Wl,-z,noexecstack -Wl,-z,now -pie -mcmodel=large -flto-jobs=8 -fuse-ld=${TOOLS_DIR}/bin/ld.lld --gcc-toolchain=/usr -fuse-ld=lld ${GCC_LINK_DIR_FLAGS} -lpthread"
ROOT_DIR=$OLDPWD/..

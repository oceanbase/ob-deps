...
# check source code
if [[ -z `find $ROOT_DIR -maxdepth 1 -regex ".*/snappy-$VERSION.*[tar|gz|bz2|xz|zip]$"` ]]; then
    echo "Download ${PROJECT_NAME} source code"
    if [[ $VERSION == "1.0.3" ]]; then
        wget --no-check-certificate https://chromium.googlesource.com/external/snappy/+archive/d8ad4b85219a45fb85a3a95d17dc8e65ef1efaad.tar.gz -O $ROOT_DIR/snappy-$VERSION.tar.gz
    else
        wget --no-check-certificate https://github.com/google/snappy/archive/refs/tags/$VERSION.tar.gz -O $ROOT_DIR/snappy-$VERSION.tar.gz
    fi
fi

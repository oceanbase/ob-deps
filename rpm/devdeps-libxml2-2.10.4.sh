...
if [[ -z `find $ROOT_DIR -maxdepth 1 -regex ".*/libxml2-$VERSION.*[tar|gz|bz2|xz|zip]$"` ]]; then
    ...
    wget --no-check-certificate https://download.gnome.org/sources/libxml2/2.10/libxml2-$VERSION.tar.xz -O $ROOT_DIR/libxml2-$VERSION.tar.xz
fi
...
cd $TMP_DIR
tar -xf $ROOT_DIR/libxml2-$VERSION.tar.xz
cd libxml2-$VERSION

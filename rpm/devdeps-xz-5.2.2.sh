...
if [[ -z `find $ROOT_DIR -maxdepth 1 -regex ".*/xz-$VERSION.*[tar|gz|bz2|xz|zip]$"` ]]; then
    ...
    wget --no-check-certificate https://nchc.dl.sourceforge.net/project/lzmautils/xz-$VERSION.tar.gz -O $ROOT_DIR/xz-$VERSION.tar.gz
fi
...
cd $TMP_DIR
tar -xf $ROOT_DIR/xz-$VERSION.tar.gz
cd xz-$VERSION

./configure --prefix=${TMP_INSTALL} --enable-static --with-pic=yes
make -j${CPU_CORES}
make install
